...
export function TemplateModal({ open, onClose, onSelect }: Props) {
  ...
  return (
    <div
      ...
      onClick={onClose}
    >
      <div className="bg-white rounded-xl shadow-2xl w-[480px] max-h-[600px] overflow-hidden" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="overflow-y-auto p-4 space-y-3">
          {WORKFLOW_TEMPLATES.map((template) => (
            ...
            >
              ...
              <div className="text-[10px] text-gray-400 mt-1">{template.nodes.length} 个节点 · {template.edges.length} 条连线</div>
            </button>
          ))}
        </div>
      </div>
